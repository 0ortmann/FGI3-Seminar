...
\]
Vergleiche \cite{huth+04} (wobei wir eine reduzierte Form der LTL Symbole verwenden, ohne direkte Definition von $F$, $G$, $R$ und $W$); "`Future"', "`Globally"' "`Release"' und "`Weak-Until"' lassen sich mit den anderen Symbolen wie folgt bilden:
\[
    F\phi := \top U\phi
\]
\[
