% !TeX program = xelatex
% !TeX spellcheck = de_DE
\documentclass[14pt]{beamer}
\usepackage{fontspec}
\usepackage{polyglossia}
\usepackage{hyperref}
\usepackage{xspace}
\usepackage{array}
\usepackage{fontawesome}
\usepackage[normalem]{ulem}
\usepackage{amsmath}

%\setsansfont{Calibri} %kann ich sonst nicht kompilieren
\setdefaultlanguage{german}
\usetheme{UHH}

\def\authors#1#2{\author[#1]{#2}}
\def\email#1{\texttt{#1}}
\input{tex/meta}
\date{25. Februar 2016}
\titlegraphic{\includegraphics[width=3cm]{img/uhh.pdf}}

\setcounter{tocdepth}{1}
\def\tocname{Gliederung des Vortrags}
\AtBeginSection{\frame{\frametitle{\tocname}\tableofcontents[currentsection]}}

\newcolumntype{x}[1]{>{\centering\arraybackslash}m{#1}}
\newtheorem{satz}{Satz}[section]

\setbeamertemplate{headline}[default]

\defbeamertemplate*{title page}{customized}[1][]
{
	{\color{leuchtrot}
	\usebeamerfont{title}\centering\inserttitle\par
	\centering\usebeamerfont{subtitle}\insertsubtitle\par}
	\bigskip
	\centering\usebeamerfont{author}\insertauthor\par
	\bigskip
	\usebeamerfont{institute}\insertinstitute\par
	\bigskip
	\usebeamerfont{date}\insertdate\par
}

\begin{document}

\fontsize{14pt}{14pt}
	
% Titelfolie
{\setbeamercolor{title page}{bg=leuchtrot}
\begin{frame}%
	\titlepage
\end{frame}}

% Gliederung
\begin{frame}{\tocname}
	\tableofcontents
\end{frame}

\include{tex/einleitung-slides}
\section{Omega Automaten}
\include{tex/omega-automaten-slides}
\include{tex/ltl-slides}
\include{tex/aba-zu-ltl-slides}
\section{LTL auf normalen Büchi Automaten}
\include{tex/ba-zu-ltl-slides} 
\include{tex/modelchecking-slides}
\section{Zusammenfassung}
\include{tex/zusammenfassung-slides}

% Literatur
\section{\bibname}
\begin{frame}[allowframebreaks]{\bibname}
	\AtBeginSection{}
	\nocite{*}
	\bibliographystyle{abbrv}
	\bibliography{bib}
\end{frame}

\end{document}